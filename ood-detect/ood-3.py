...
from trust.utils.organmnist3 import load_dataset_custom_aid as load_dataset_custom
...
budget=30
num_epochs = int(10)
split_cfg = {'num_cls_idc':num_cls, 'per_idc_train':10, 'per_idc_val':0, 'per_idc_lake':150, 'per_ood_train':0, 'per_ood_val':1, 'per_ood_lake':1000}

initModelPath = "/mnt/data2/akshit/AL-OOD/ood-detect/weights/" + data_name + "_" + feature + "_" + model_name + "_" + str(learning_rate) + "_" + str(split_cfg["per_idc_train"]) + "_" + str(split_cfg["per_idc_val"]) + "_" + str(split_cfg["num_cls_idc"])
        ...
        if(i==0):
            ...
            if(strategy=="SIM" or strategy=="SF"):
                if(sf.endswith("mi")):
                    ...
                    strategy_sel.update_queries(val_set) #In-dist samples are in Val, Out-dist are in private
...
#             print("Hypothesized targets of subset: ", hypothesized_targets)
            ...
            if(feature=="classimb"):
                ...
                train_set = torch.utils.data.ConcatDataset([train_set, add_val_set]) #Add the OOD samples with a common OOD class
                val_set = ConcatWithTargets(val_set, new_private_set)
                if(len(private_set)!=0):
                    private_set = ConcatWithTargets(private_set, add_val_set)
                else:
                    private_set = add_val_set
            else:
